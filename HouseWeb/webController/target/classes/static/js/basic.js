...
//延迟显示图片
    ...
    $(document).on('click', '.search_w>li>a', function(){
        ...
        var data=setHistoryData($(this).attr("href"), $(this).attr("x"), $(this).attr("y"), names[0], parseInt(names[1]));
...
if( !('placeholder' in document.createElement('input')) ){
	...
	//列表房源点击之后 标题变灰(图片)
	$(".pList li .listImg").bind('click', function(){
		var beforeli = $(this).parent();
		if(beforeli){
            beforeli.find(".listTit a").css("color","#96a1ae");
		}
	});
    //列表房源点击之后 标题变灰(标题)
	$(".listTit a").bind('click', function(){
        $(this).css("color","#96a1ae");
	});
});
...
//右侧 下载APP 跟着屏幕滚动
if($(".rBox").length>0){
	var toTop = $(".list_code").offset().top;
	var footTop = $(".ind-friend-link").offset().top - $(".new-box").height();
	var rightDownload = $(".list_code").height() +40;
	var toTopagentcont = footTop - rightDownload;
	$(window).scroll(function() {
		if(toTop < $(document).scrollTop() && $(document).scrollTop() < toTopagentcont) {
			$(".list_code").addClass("tagrightapp");
			$(".list_code").css({"position":"fixed","top":"0px","bottom":"auto"});
		}else if($(document).scrollTop() <= toTop){
			$(".list_code").css("position","inherit");
		}else{
			var bottom1=window.scrollY;
			var bottom2=document.documentElement.clientHeight;
			var bottom_ath=bottom1-footTop+bottom2+40;
			
			var bottom_ath1 = $(".new-box").height() + $(".ind-friend-link").height() + $(".footer-box").height() + 40 ;
			$(".list_code").css({"position":"fixed","top":"auto","bottom":bottom_ath1+"px"});
		}
	});
}
