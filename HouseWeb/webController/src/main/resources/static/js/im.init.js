...
var cookieToken = "webim_token_"+contactUserId;
var webToke = getCookieByName(cookieToken);

if (isActived && webToke != undefined && webToke != ' ' && contactUserId > 0){
    var options = {
        apiUrl: WebIM.config.apiURL,
    ...
    conn.open(options);
}else if(isActived && contactUserId > 0) {
    var options = {
        apiUrl: WebIM.config.apiURL,
        user: contactUserId+'uid', //账户
        ...
        success: function (token) {
            ...
            WebIM.utils.setCookie('webim_token_'+contactUserId, token, 1);
        },
        error: function(){
        }
    };
