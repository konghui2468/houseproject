...
//	 });
	...
	$(document).on( "click" , ".aBtn" ,function(){
		...
		$(this).parent().siblings(".bqBox").children(".imgIcon").children("input").removeAttr("disabled");
		$(this).parent().siblings(".bqBox").children(".bqBox .imgIcon").css("opacity",1);
	});
...
function appendMsg(data,type) {
    ...
    //2018-11-5 新房楼盘卡片兼容 libaisong
    if (data.type == 'kh' && data.subType  == 'card1') {
        //var str = '<div class="liaoBox kh"><div style="float: right"><div class="kh-headCon right-bg"> <div class="kh-headBg"><a target="_blank" href="'+ weburl +'"><div class="fySty"><img src="'+ houseInfo.imgurl+'" onerror="houseimgerror(this)"><p>'+ houseInfo.title +'</p> <p><span>'+ houseInfo.area +'</span></p><p><strong>'+ houseInfo.price +'</strong></p></div></a></div> </div> <div class="kh-head"><img src="'+ data.userPic +'"></div></div></div>';
        var str = '<div class="liaoBox kh"> <div style="float: right"> <div class="kh-headCon right-bg"> <div class="kh-headBg"> <a target="_blank" href="http://fang.5i5j.com'+ data.EstateUrl +'">  <div class="fySty"> <img src="'+ data.agency_building_url+'"> <p>'+ data.agency_building_name +'</p> <p><span>'+ data.agency_building_area +'</span></p></div> </a>    </div> </div> <div class="kh-head"><img src="'+ data.userPic +'"></div> </div> </div>';
    }
...
function showMessagelist(messageInfoList) {
    ...
    if (typeof messageInfoList.total !== 'undefined' && data.length > 0) {
        for (var i = 0; i < data.length; i++) {
            var oldTime = (new Date(replace_data(data[i].in_date))).getTime();
            if (i > 0) {
                var curTime = (new Date(replace_data(data[i - 1].in_date))).getTime();
            ...
            if (data[i].conversation_type == 1) {
                ...
                //2018-11-5 新房楼盘卡片兼容 libaisong
                if (data[i].message_type == 11) {
                    var msgdata = JSON.parse(data[i].message_extend_content);
                    appendData = {
                        "type": "kh",
                        "subType": "card1",
                        "toReceiver": "messageBox_" + toReceiver,
                        "EstateUrl": msgdata.PcEstateUrl,
                        "agency_building_url": msgdata.agency_building_url,
                        "agency_building_name": msgdata.agency_building_name,
                        "agency_building_area": msgdata.agency_building_area,
                        "userPic": userPic,
                        "scrollTop" : messageInfoList.scrollTop,
                        "fromType":'kh'
                    };
...
function formatTime(time){
	...
	if(time - today <= 24*3600*1000 && time - today >=0){
		...
		return "昨天 " + addZero(t.getHours()) + ":" + addZero(t.getMinutes()) ;
...
function replace_data(source_data){
    var source_time = source_data.replace(/\-/g, "/");
    return source_time.substring(0,19);
}
...
function addBroker(agencyID, cityID, msg, messageType, useUserPic) {
    getBrokerInfo(agencyID, cityID, '', msg, messageType, useUserPic)
}

/**** 南昌数据融合后查询经纪人信息需要增加citycode参数 *****/
function newAddBroker(agencyID, cityID, cityCode, msg, messageType, useUserPic) {
    getBrokerInfo(agencyID, cityID, cityCode, msg, messageType, useUserPic)
}

function getBrokerInfo (agencyID, cityID, cityCode, msg, messageType, useUserPic) {
    if(contactUserId == 0) {
	...
	if(undefined == agentLiReceiveID) {
        if(message.from.indexOf('uid') >= 0) {
            if(message.messageType == 1) {
                addBroker(agentID,cityId,JSON.stringify(message.ext.HOUSE_INFO),message.messageType, 1);
            } else {
                addBroker(agentID,cityId,message.data,message.messageType, 1);
            }
        } else {
            if(message.messageType == 1) {
                addBroker(agentID,cityId,JSON.stringify(message.ext.HOUSE_INFO), message.messageType);
            } else {
                addBroker(agentID,cityId,message.data, message.messageType);
            }
        }
...
function clientSendMessage(sendType) {
    ...
    //console.log(1);
    if( !isActived) {
        var messageBox = '<div class="news-box" style="text-align: center; margin-bottom: 0px" onclick="scrollButtom()">您已被禁言</div>';
        $("#messageBox_"+toReceiver).append(messageBox);
        var timeout=setTimeout(function () {
            $(".news-box").fadeOut(1000);
        }, 2000);
    }else if($('#'+toReceiver).attr('leave') == 0) {
